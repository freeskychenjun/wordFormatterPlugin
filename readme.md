...
对所有匹配到的段落只写"段落直接格式"，不修改文档样式定义（`styles.xml`）和文档主题。这样保证用户文档原有的样式表保留完整，可重新打开后继续编辑。

针对 OOXML 几个会"无声覆盖"段落直接格式的属性，写入用户值前先清零：

| 用户设置 | 同时清零 | 原因 |
|---------|--------|------|
| 段前 (SpaceBefore) | LineUnitBefore、SpaceBeforeAuto | 行单位间距和自动间距优先级高于点值 |
| 段后 (SpaceAfter) | LineUnitAfter、SpaceAfterAuto | 同上 |
| 段前/段后 | ContextualSpacing | 同样式相邻段间距会被压缩 |

## 标题自动编号处理

像"第一章 概述"中的"第一章"由 numbering.xml 渲染，不在文档文本流里——光标选不到、段落字体也覆盖不到。排版时对识别为标题的段落自动调用 `ListFormat.ConvertNumbersToText()` 把编号转成普通文本，与正文一同接受字体设置；并在编号末尾补一个空格作为分隔。

**代价**：被转换的标题失去自动重编号能力（新增/删除章节时编号不会自动重排）。如果需要保留可重编号，请在排版前手动备份文档。
